        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            ...
            <div className="relative">
              {avatarUrl && !avatarFailed ? (
                <img
                  ...
                  className="w-24 h-24 rounded-full object-cover border-4 border-sky-50 shadow-sm"
                />
              ) : (
                <div className="w-24 h-24 rounded-full border-4 border-sky-50 bg-sky-100 text-sky-700 flex items-center justify-center text-2xl font-bold shadow-sm">
                  {initials}
                </div>
              )}
              <span className="absolute bottom-1 right-1 w-4 h-4 rounded-full bg-emerald-500 border-2 border-white" />
            </div>
            <label className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs font-semibold text-sky-700 cursor-pointer hover:bg-sky-50 hover:border-sky-200 transition-colors">
          ...
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              ...
              <span className="self-start px-3 py-1.5 rounded-lg bg-sky-50 text-sky-700 border border-sky-200 text-xs font-semibold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-7">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <button type="submit" disabled={saving} className="btn-primary w-full h-11 text-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">{saving ? 'Saving Changes...' : 'Save Changes'}</button>
          </form>
        </section>

        <div className="space-y-6">
          <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-7">
            <div className="mb-5"><h3 className="text-base font-bold text-slate-900 flex items-center gap-2"><School className="w-4 h-4 text-sky-600" /> Institutional Record</h3><p className="text-xs text-slate-500 mt-1">Read-only account information managed by the institution.</p></div>
            <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
              <div className="p-4 flex items-center justify-between gap-4"><span className="text-sm text-slate-500">Assigned Role</span><span className="text-sm font-semibold text-slate-800 text-right">{ROLE_NAMES[currentUser.role]}</span></div>
              <div className="p-4 flex items-center justify-between gap-4"><span className="text-sm text-slate-500">Affiliation Type</span><span className="text-sm font-semibold text-slate-800 text-right">{currentUser.userType}</span></div>
              <div className="p-4 flex items-center justify-between gap-4"><span className="text-sm text-slate-500">Account Status</span><span className="inline-flex items-center gap-2 text-sm font-semibold text-emerald-700"><span className="w-2 h-2 rounded-full bg-emerald-500" />Active & Verified</span></div>
              <div className="p-4 flex items-center justify-between gap-4"><span className="text-sm text-slate-500">Academic Year</span><span className="text-sm font-semibold text-slate-800">AY 2026–27</span></div>
            </div>
          </section>

          <section className="bg-sky-50/70 border border-sky-100 rounded-2xl p-5 flex items-center justify-between gap-4">
            <div><p className="text-sm font-bold text-sky-900 flex items-center gap-2"><Settings className="w-4 h-4 text-sky-600" /> Security & Settings</p><p className="text-xs text-sky-700 mt-1">Update password, notification alerts, or sign out.</p></div>
            <Link to="/settings" className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white hover:bg-sky-100 text-sky-700 border border-sky-200 text-xs font-semibold transition-colors flex-shrink-0">Go to Settings <ArrowRight className="w-3.5 h-3.5" /></Link>
          </section>
        </div>
      </div>
    </div>
